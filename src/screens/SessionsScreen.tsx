  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={load} colors={[COLORS.primary]} />}>
      <View style={styles.card}>
        ...
        {loading && sessions.length === 0 ? <SessionsSkeleton /> : sessions.length === 0 ? <Text style={styles.empty}>No active sessions found.</Text> : sessions.map((session, index) => {
...
const makeStyles = (COLORS: AppColors) => StyleSheet.create({container: {flex: 1, backgroundColor: COLORS.bg}, content: {padding: 16}, card: {backgroundColor: COLORS.card, borderRadius: 15, borderWidth: 1, borderColor: COLORS.border, padding: 17}, header: {flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}, title: {fontSize: 21, fontWeight: '900', color: COLORS.text}, subtitle: {fontSize: 11, color: COLORS.textSecondary, marginTop: 4}, refresh: {fontSize: 25, color: COLORS.primary}, loader: {marginVertical: 25}, empty: {fontSize: 13, color: COLORS.textMuted, textAlign: 'center', paddingVertical: 30}, row: {flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: COLORS.border}, icon: {width: 38, height: 38, borderRadius: 19, backgroundColor: COLORS.surface, alignItems: 'center', justifyContent: 'center'}, iconText: {color: COLORS.text}, copy: {flex: 1}, device: {fontSize: 13, fontWeight: '800', color: COLORS.text}, meta: {fontSize: 10, color: COLORS.textMuted, marginTop: 4}, endButton: {backgroundColor: COLORS.dangerLight, borderRadius: 7, paddingHorizontal: 10, paddingVertical: 7}, endText: {fontSize: 10, fontWeight: '800', color: COLORS.danger}, logoutAll: {borderWidth: 1, borderColor: COLORS.danger, borderRadius: 9, alignItems: 'center', paddingVertical: 12, marginTop: 18}, logoutText: {color: COLORS.danger, fontWeight: '800', fontSize: 12}});
